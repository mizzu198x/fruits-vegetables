...
class VegetableController extends AbstractController
{
    ...
    #[OA\Tag(name: 'vegetables-operation')]
    #[OA\Query(requestBody: new OA\RequestBody(ref: new Model(type: SearchPlantRequest::class)))]
    #[OA\Response(response: 200, description: 'Vegetables returned')]
    #[Security(name: 'basic')]
    #[Route('/api/v0/vegetables', name: 'api_vegetables_list', methods: ['GET'])]
    ...
    #[OA\Response(response: 201, description: 'Vegetable added')]
    #[OA\Response(response: 409, description: 'Vegetable already existed')]
    #[Security(name: 'basic')]
    #[Route('/api/v0/vegetable', name: 'api_vegetables_add', methods: ['POST'])]
    ...
    public function add(CreatePlantRequest $request): Response
    {
        ...
        if ($plant instanceof Plant) {
            throw new ResourceConflictException('Vegetable already existed');
        }

        $plant = $this->dataProcessor->convertRequestToEntity($request, PlantType::VEGETABLE);
    ...
    #[OA\Response(response: 202, description: 'Vegetable updated')]
    #[OA\Response(response: 404, description: 'Vegetable not found')]
    #[Security(name: 'basic')]
    #[Route('/api/v0/vegetable/{id}', name: 'api_vegetables_update', methods: ['PATCH'])]
    #[IsGranted('ROLE_ADMIN')]
    public function update(int $id, UpdatePlantRequest $request): Response
    {
        $plant = $this->plantRepository->find($id);
        if (!$plant instanceof Plant) {
            throw new ResourceNotFoundException('Vegetable not found');
        }
    ...
    #[OA\Tag(name: 'vegetables-operation')]
    #[OA\Response(response: 204, description: 'Vegetable deleted')]
    #[OA\Response(response: 404, description: 'Vegetable not found')]
    #[Security(name: 'basic')]
    #[Route('/api/v0/vegetable/{id}', name: 'api_vegetables_delete', methods: ['DELETE'])]
    #[IsGranted('ROLE_ADMIN')]
